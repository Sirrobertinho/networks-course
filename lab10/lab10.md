...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   - 20 байт
   - 28 байт ( 20 байт (размер IP заголовка) + 8 байт (размер UDP заголовка) + 28 байт (полезная нагрузка) = 56 байт (размер пакета) )
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Version, Header Length, DSField, Total length, Flags, Protocol, Source Address, Destination Address
      - Все из предыдущего вопроса
      - TTL, Identification, Header Checksum
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Очень случайно
      - Если она есть при запуске на Unix, то я её не нашёл (Однако при запуске traceroute -I akamai.com 56 (флаг -I похволяет делать трассировку с помошью ICMP) поле Identification увеличивается на 1 у следующего отправленного пакета)
![](/images/lab10/2.png)
5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)? (я выбрал первый)
   - 0x6c73 
   - 1
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - TTL остаётся неизменным (64), а Identification увеличивается на 1 с каждым сообщением
![](/images/lab10/3.png)
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL? (выбрал пакет из предыдущего вопроса)
   - 0xcfae
   - 64
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Было создано 3 фрагмента
   ![](/images/lab10/4.png)
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Total Length (у последнего меньше), More fragments (у последнего нет), Fragment offset, Header checksum
  ![](/images/lab10/5.png)
