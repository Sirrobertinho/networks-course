...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Очень случайно
      - Если она есть при запуске на Unix, то я её не нашёл (Однако при запуске traceroute -I akamai.com 56 (флаг -I похволяет делать трассировку с помошью ICMP) поле Identification увеличивается на 1 при каждом запросе)
![](/images/lab10/2.png)
