...
2. Определите длину (в байтах) для каждого поля UDP-заголовка, обращаясь к отображаемой
   ...
   - 2 байта Source Port, 2 байта Destination Port, 2 байта Length, 2 байта Checksum (это можно понять выделив соответствующее поля и посмотреть на выделившееся байты)
3. Значение в поле Length (Длина) – это длина чего?
   - Длина UDP заголовка - 8 байт + длина UDP данных (измеряется в байтах)
4. Какое максимальное количество байт может быть включено в полезную нагрузку UDP-пакета?
   - На Length выделяется 2 байта, то есть максимум 65535 (= $2^{16} - 1$) байта. 8 байт уходит на заголовок, значит полезная нагрузка максимум 65535 - 8 = 65527 байт 
5. Чему равно максимально возможное значение номера порта отправителя?
   - На Source Port выдеояется 2 байта, значит максимальный номер 65535
...
7. Проверьте UDP-пакет и ответный UDP-пакет, отправляемый вашим хостом. Определите
   ...
   - Номера портов меняются местами
